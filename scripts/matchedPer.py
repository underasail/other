...
with open(argv[1], newline='') as f:
    ...
    for row in csvreader:
        if len(row) >= 14 and 'XM:i:0' in str(row):
            ...
            pass
primary_matched_per = round((len(primary_matched_dict.keys())*100)/totalreads, 2)
...
with open(argv[2], newline='') as f:
    ...
    for row in csvreader:
        if len(row) >= 14 and 'XM:i:0' in str(row):
            ...
            pass
secondary_matched_per = round((len(secondary_matched_dict.keys())*100)/totalreads, 2)
    # percentage of reads that mapped to both primary and secondary
secondary_unmatched_per_total = \
round((len(secondary_unmatched_dict.keys())*100)/totalreads, 2)
    # percentage of primarily unmatched reads that map to secondary
secondary_unmatched_per_cont = \
round((len(secondary_unmatched_dict.keys())*100)/primary_unmatched_num, 2)
    # percentage of primarily unmatched reads that map to secondary
