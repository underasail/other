...
for key, value in refdict.items():
    ...
    # print('%s countains %d matched reads.' % (key, len(value)))
...
for key, value in refdict_count.items():
    ...
    # print('%s represents %0.2f%% of the reads.' % (key, percent))
    refdict_per.setdefault(key, []).append(percent)
# Sums total reads caught and generates a percent for each reference genome
# Prints out each percentage and associated ref genome along with setting up another dictionary to house percentages
